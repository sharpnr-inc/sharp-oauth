...
use crate::{
    ...
    oauth::{
        repo::{
            authorization_codes, clients,
            scopes::{self, ScopeDescription},
        },
        services::{client::OAuthClient, consent, params::Params, pkce, scope::ScopeSet},
    },
    shared::{
        database::SecretHash,
        error::AppError,
...
/// ([`crate::oauth::models::oauth_authorization_codes`]). `code_hash` is a
/// [`SecretHash`], so `{:?}` prints `<redacted>`.
pub use crate::oauth::models::oauth_authorization_codes::Model as AuthorizationCode;
...
pub async fn validate(
    ...
    let client = clients::find_by_client_id(db, client_id)
        .await?
...
pub async fn decide(
    ...
    let scopes = scopes::find_by_names(&state.db, &validated.scope.to_vec()).await?;
...
async fn issue_code(
    ...
    authorization_codes::insert(&state.db, &record).await?;
...
mod tests {
    ...
    fn max_age_forces_reauthentication_for_old_sessions() {
        ...
        let session = Session {
            id: Uuid::now_v7(),
            user_id: Uuid::now_v7(),
            session_token_hash: crate::shared::database::SecretHash::from("hash".to_owned()),
            revoked_at: None,
