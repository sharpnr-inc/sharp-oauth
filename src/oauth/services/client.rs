...
use crate::{
    ...
    oauth::{
        repo::{clients, scopes},
        services::{params::Params, scope::ScopeSet},
    },
    shared::{
        database::SecretHash,
        error::AppError,
...
/// ([`crate::oauth::models::oauth_clients`]). `client_secret_hash` is a
/// [`SecretHash`], so `{:?}` prints `<redacted>`.
pub use crate::oauth::models::oauth_clients::Model as OAuthClient;
...
pub async fn authenticate(
    ...
    let client = clients::find_by_client_id(db, &credentials.client_id)
        .await?
