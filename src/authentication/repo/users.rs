...
use crate::authentication::{
    models::users::{Column, Entity as Users},
    services::user::User,
};

/// Fails with a unique-violation error if the email is already taken; see
/// [`crate::authentication::services::user::sign_up`], which relies on that.
...
/// `email` must already be normalised (see [`crate::authentication::services::user::normalize_email`]).
