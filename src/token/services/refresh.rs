...
/// ([`crate::token::models::oauth_refresh_tokens`]). `token_hash` is a
/// [`SecretHash`], so `{:?}` prints `<redacted>`.
pub use crate::token::models::oauth_refresh_tokens::Model as RefreshToken;
...
pub async fn issue(
    ...
    refresh_tokens::insert(db, &record).await?;
    Ok((raw, record))
}
