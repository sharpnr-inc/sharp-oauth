...
pub async fn handle(
    ...
    let Some(record) = db::refresh_tokens::find_by_hash(&state.db, &hash_token(token)).await?
    else {
        return Ok(());
    };
    ...
    let revoked =
        db::refresh_tokens::revoke_family(&state.db, record.family_id, Utc::now()).await?;
    tracing::info!(
        target: "audit",
