...
use crate::{
    AppState,
    authentication::{
        dtos::{CsrfForm, ReturnToQuery, SignInForm, SignUpForm},
        services::{
            session::{self, SESSION_COOKIE, SESSION_TTL},
            user::{self, NewAccount, SignUpError, User},
        },
    },
    middlewares::{
        auth::CurrentSession,
...
pub async fn home(
    ...
    with_csrf_cookie(views::home_page(user, &csrf.value), csrf)
}
...
pub async fn sign_in_page(
    ...
    let page = views::sign_in_page(StatusCode::OK, &csrf.value, return_to, "", None);
...
pub async fn sign_up_page(
    ...
    let csrf = csrf::issue(&headers, state.config.secure_cookies());
    let return_to = query.return_to.as_deref().map(safe_return_to);
    let page = views::sign_up_page(StatusCode::OK, &csrf.value, return_to, "", "", None);
    with_csrf_cookie(page, csrf)
}
...
pub async fn logout(
    ...
    if let Some(token) = cookie_manager::get(&headers, SESSION_COOKIE)
    ...
    response.headers_mut().append(
        header::SET_COOKIE,
        cookie_manager::clear(SESSION_COOKIE, state.config.secure_cookies()),
    );
    response
}
...
async fn start_session(
    ...
    if let Some(old) = cookie_manager::get(headers, SESSION_COOKIE)
