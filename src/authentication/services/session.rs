...
/// ([`crate::authentication::models::user_sessions`]). `session_token_hash` is a
/// [`SecretHash`], so `{:?}` prints `<redacted>`.
pub use crate::authentication::models::user_sessions::Model as Session;
...
pub async fn find_active(
    ...
    let Some(session) = sessions::find_active_by_hash(db, &hash_token(token), Utc::now()).await?
    else {
        return Ok(None);
    };
    let Some(user) = users::find_by_id(db, session.user_id).await? else {
        return Ok(None);
    };
...
pub async fn revoke(db: &DatabaseConnection, token: &str) -> Result<(), AppError> {
    sessions::revoke_by_hash(db, &hash_token(token), Utc::now()).await?;
    Ok(())
}
